...
class SearchResults extends Component {
    ...
    handleClick(e){
        ...
        const proxifyUrlIsbn=proxify(encodeURI(ENV.ISBN_API+e.target.getAttribute('name')+'?key='+ENV.KEY), { inputFormat: "xml" });
        axios.get(proxifyUrlIsbn)
        .then(response => {
    ...
    render(){
        return(
            <div className="search-results-list">
                {
                    (this.props.searchResults.length === 0 && !this.props.searchingList)?'':(this.props.searchingList)?<h2 className="text-center">Searching...</h2>:<ul className="search-result-ul">
                    {
                        this.props.searchResults.map((ele, idx) => {
                            return (<li className="search-result-li" key={idx} name={ele.best_book.id.content} onClick={this.handleClick}>{ele.best_book.title}</li>)
                        })
                    }
                </ul>
                }
